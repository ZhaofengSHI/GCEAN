...
def collate_fn_train(batch):
    # feats, gt_featstamps, action_labels, caption_label, gt_timestamps, duration, captions, key ,feats_view2, key_view2,  gaze_feats, gaze_feats_view2

    batch_size = len(batch)

    # video
    feature_size = batch[0][0].shape[1]
    feature_size_view2 = batch[0][-4].shape[1]

    # gaze
    feature_size_gaze = batch[0][-2].shape[1]
    feature_size_gaze_view2 = batch[0][-1].shape[1]

    feature_list, gt_timestamps_list, labels, caption_list, gt_raw_timestamp, raw_duration, raw_caption, key, feature_list_view2, key_view2, gaze_feature_list, gaze_feature_list_view2 = zip(
        *batch)

    # video
    max_video_length = max([x.shape[0] for x in feature_list])
    max_video_length_view2 = max([x.shape[0] for x in feature_list_view2])

    # gaze
    max_video_length_gaze = max([x.shape[0] for x in gaze_feature_list])
    max_video_length_gaze_view2 = max([x.shape[0] for x in gaze_feature_list_view2])
    ...
    video_tensor_view2 = torch.FloatTensor(batch_size, max_video_length_view2, feature_size_view2).zero_()
    video_mask_view2 = torch.BoolTensor(batch_size, max_video_length_view2).zero_()

    ## gaze video feature
    video_tensor_gaze = torch.FloatTensor(batch_size, max_video_length_gaze, feature_size_gaze).zero_()
    video_mask_gaze = torch.BoolTensor(batch_size, max_video_length_gaze).zero_()

    video_tensor_gaze_view2 = torch.FloatTensor(batch_size, max_video_length_gaze_view2,
                                                feature_size_gaze_view2).zero_()
    video_mask_gaze_view2 = torch.BoolTensor(batch_size, max_video_length_gaze_view2).zero_()
    ...
    for idx in range(batch_size):
        ...
        video_len_view2 = feature_list_view2[idx].shape[0]

        ## gaze video len
        video_len_gaze = gaze_feature_list[idx].shape[0]
        video_len_gaze_view2 = gaze_feature_list_view2[idx].shape[0]
        ...
        # video
        video_tensor_view2[idx, :video_len_view2, :] = torch.from_numpy(feature_list_view2[idx])
        video_mask_view2[idx, :video_len_view2] = True

        ## gaze
        video_tensor_gaze[idx, :video_len_gaze, :] = torch.from_numpy(gaze_feature_list[idx])
        video_mask_gaze[idx, :video_len_gaze] = True

        video_tensor_gaze_view2[idx, :video_len_gaze_view2, :] = torch.from_numpy(gaze_feature_list_view2[idx])
        video_mask_gaze_view2[idx, :video_len_gaze_view2] = True
        ...
        "video":
            {
                ...
                "tensor_view2": video_tensor_view2,
                "mask_view2": video_mask_view2,
                "key_view2": list(key_view2),

                ## gaze
                "tensor_gaze": video_tensor_gaze,
                "mask_gaze": video_mask_gaze,

                ## gaze view2
                "tensor_gaze_view2": video_tensor_gaze_view2,
                "mask_gaze_view2": video_mask_gaze_view2
            },
        # "lnt":
        #     {
...
def collate_fn_eval(batch):
    # feats, gt_featstamps, action_labels, caption_label, gt_timestamps, duration, captions, key
...
class EDVCdataset(Dataset):
    ...
    def __init__(self, anno_file, feature_folder, translator_json, is_training, proposal_type, opt,
        ...
        self.gaze_feature_folder = gaze_feature_folder
...
###
####
class DVC_DA_Dataset(EDVCdataset):

    def __init__(self, anno_file, feature_folder, translator_pickle, is_training, proposal_type,
                 opt, gaze_feature_folder=None):
        super(DVC_DA_Dataset, self).__init__(anno_file,
                                             feature_folder, translator_pickle, is_training, proposal_type,
                                             opt, gaze_feature_folder)
    ...
    def load_gaze_feats(self, key):
        vf_types = self.opt.visual_feature_type
        rescale_method = 'fix_length'
        if type(vf_types) == list:
            assert type(self.gaze_feature_folder) == list and len(vf_types) == len(self.gaze_feature_folder)
            feats_dict = {}
            all_padding = True
            for vf_type, vf_folder in zip(vf_types, self.gaze_feature_folder):
    ...
    def __getitem__(self, idx):
        ...
        if self.is_training:
            ...
            if key.split('_')[-1] == 'ego':
                key_view2 = key[:-3] + 'exo'
            elif key.split('_')[-1] == 'exo':
                key_view2 = key[:-3] + 'ego'
            feats_view2 = self.load_feats(key_view2)

            if self.gaze_feature_folder:
                gaze_feats = self.load_gaze_feats(key)
                gaze_feats_view2 = self.load_gaze_feats(key_view2)

            assert key_view2 != None
            assert len(feats_view2)
            assert len(gaze_feats)
            assert len(gaze_feats_view2)

            return feats, gt_featstamps, action_labels, caption_label, gt_timestamps, duration, captions, key, feats_view2, key_view2, gaze_feats, gaze_feats_view2
