...
def eval_metrics(dvc_filename, gt_filenames, para_gt_filenames, alpha=0.3, temperature=2.0, cl_score_weight=0.,
    ...
    dvc_score.update({'MetaScore': dvc_score['Bleu_4'] * 5.0 + dvc_score['METEOR'] + dvc_score['CIDEr'] + dvc_score['SODA_Meteor'] + dvc_score['SODA_Cider'] + dvc_score['SODA_tIoU']})
...
def evaluate(model, memory_bank, criterion, contrastive_criterion, postprocessors, loader, dvc_json_path, logger=None,
    ...
    # print(opt.gt_file_for_para_eval)

    # print(opt.count_loss_coef)
    if opt.count_loss_coef > 0:
        dvc_json_path_new = reranking(dvc_json_path, alpha=alpha, cl_score_weight=opt.eval_matching_score_weight,
                                      temperature=2.0)

    save_dvc_json(out_json_g, dvc_json_path_new + '.grounding.json')
    save_dvc_json(aux_out_json_g, dvc_json_path_new + '_aux.grounding.json')
    skip_lang_eval = skip_lang_eval or vars(opt).get('eval_disable_captioning', False)
    if not skip_lang_eval:
        scores = eval_metrics(dvc_json_path_new,
    ...
    scores_g = eval_metrics_grounding(dvc_json_path_new + '.grounding.json', gt_filename=opt.eval_gt_file_for_grounding)
    aux_scores_g = eval_metrics_grounding(dvc_json_path_new + '_aux.grounding.json',
